...
class Triangle:
    ...
    def get_maxsum(self):
        if self.size > 1:
            self.up_t.get_maxsum()
            self.my_row[0] += self.up_t.my_row[0]
            self.my_row[-1] += self.up_t.my_row[-1]
            for i in range(-2, -self.size, -1):
                self.my_row[i] += max(self.up_t.my_row[i], self.up_t.my_row[i + 1])
            self.maxsum.extend(self.my_row)
            return
        else:
            self.maxsum.extend(self.my_row)
            return
